...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      if (Global.incompleteProfile && Global.completeLater == false) await _showIncompleteProfileDialog();
  ...
  void didChangeDependencies() {
    if (_isInit) {
      Provider.of<JobProvider>(context, listen: false).fetchSliderImages().then((value) {
        Provider.of<Auth>(context, listen: false).currentUser.isCustomer
            ? Provider.of<Customer>(context, listen: false).getAllCategories().then(
                  (value) => setState(() {
                    _isInit = false;
                  }),
                )
            : Provider.of<JobProvider>(context, listen: false).getAllJobs().then(
                  (value) => setState(() {
                    _isInit = false;
                  }),
                );
  ...
  Future<void> _showSubCategoryDialog() async {
    return showDialog(
      context: context,
      builder: (context) {
        List<Category> subCategories = Provider.of<Customer>(context).subCategoriesList;
              ...
              child: InkWell(
                onTap: () {
                  if (category.subCategories > 0) {
                    Provider.of<Customer>(context, listen: false).getSubCategories(category.id);
                    _showSubCategoryDialog();
                  } else {
                    setState(() {
                      _catName = category.name;
                    });
                    Provider.of<Customer>(context, listen: false).filterByCategories(category.id);
                  }
                },
                child: Stack(
                  children: <Widget>[
                    ...
                    Positioned(
                      ...
                      child: FittedBox(
                        ...
                        child: Text(
                          ...
                          style: Theme.of(context).textTheme.headline6.copyWith(color: Theme.of(context).primaryColor, fontSize: 12),
                        ),
                      ),
                    ),
                  ],
    ...
    categoriesItems.insert(
      ...
      Padding(
        ...
        child: InkWell(
          onTap: () {
            setState(() {
              _catName = 'All Services';
            });
            Provider.of<Customer>(context, listen: false).filterByCategories('all');
          },
          child: ClipRRect(
            borderRadius: BorderRadius.all(Radius.circular(8.0)),
            child: Stack(
      ...
      child: Container(
        ...
        child: Scaffold(
          ...
          floatingActionButton: user.isCustomer
              ...
              : FloatingActionButton.extended(
                  onPressed: () {
                    Navigator.of(context).pushNamed(ArtistAppliedJobsScreen.routeName);
                  },
                  label: Text(
                    'Applied Jobs',
                    style: Theme.of(context).textTheme.headline6.copyWith(color: Colors.white),
          ...
          appBar: AppBar(
            centerTitle: true,
            title: Text(user.isCustomer ? 'Ipaayos Mo Customer' : 'Ipaayos Mo Provider'),
            leading: Builder(
              builder: (context) {
                return IconButton(
