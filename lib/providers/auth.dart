...
class Auth with ChangeNotifier {
  ...
  Future<void> updateQualification(String id, String newTitle, String newDesc) async {
  ...
  Future<String> updateArtistHeadInfo(String newName, String newMobile, String newGender) async {
  ...
  Future<void> setAddress(Map<String, String> newData) async {
    ...
    try {
      ...
      if (response.statusCode >= 400 || body['message'] != 'Profile has been updated successfully') {
        ...
        final prefs = await SharedPreferences.getInstance();
        prefs.setString('userData', json.encode(body['data'])).then((value) => print('new User:${body['data']}'));
      }
    } catch (err) {
      _currentUser.address = oldAddress;
      _currentUser.officeAddress = oldOffice;
  ...
  Future<void> editPersonalInfo(String newName, String newMobile, String newGender) async {
  ...
  Future<void> login(String email, String password, String deviceType, bool isCustomer) async {
    ...
    try {
      ...
      print('res: $email $password $deviceType $firebaseToken, ${isCustomer ? '2' : '1'}');
      print('response: $responseData');
      if (responseData['message'] != null && responseData['message'] == 'User login successfully') {
        prefs.setString('userData', json.encode(responseData['data']));
        final user = User(
        ...
        if(isCustomer) {
          ...
          if (user.name == '' || user.liveLat == '' || user.liveLong == '' || user.address == '') {
            Global.incompleteProfile = true;
            Global.user = user;
          }
        } else {
          print('autologin of a service provider');
          if(user.name == '' || user.liveLat == '' || user.liveLong == '' || user.mobile == ''){
            Global.incompleteProfile = true;
            Global.user = user;
          }
        }
  ...
  Future<void> signup({String name, String email, String password, String deviceType, bool isCustomer, String referralCode=''}) async {
    ...
    try {
      ...
      if (responseData['message'] != null){
            if(responseData['message'].toString() == 'Please check your email and your verified account.') {
              print("User registered msg #1");
            }else if(responseData['message'].toString().contains("registered successfully")){
